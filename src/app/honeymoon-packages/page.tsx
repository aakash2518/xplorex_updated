...
import { Clock, MapPin, Heart, ArrowUpRight } from "lucide-react";
import Link from "next/link";
import Image from "next/image";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { getHoneymoonPackages } from "@/data/destinations";

export default function HoneymoonPackagesPage() {
  const honeymoonPackages = getHoneymoonPackages();
        ...
        <div className="container px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <h1 className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-primary mb-4 sm:mb-6">
              Honeymoon <span className="text-gradient">Packages</span>
            </h1>
            <p className="text-primary/60 text-base sm:text-lg md:text-xl font-medium leading-relaxed px-4 sm:px-0">
              Celebrate your love with romantic getaways designed for couples. From tropical beaches to 
              mountain retreats, create unforgettable memories on your perfect honeymoon.
            </p>
          </motion.div>
        </div>
      </section>
            ...
            {honeymoonPackages.map((tour, i) => (
              ...
              >
                <div className="relative h-48 sm:h-56 lg:h-60 overflow-hidden">
                  <Image 
                    src={tour.img || "/assets/dest-maldives.jpg"} 
                    alt={tour.title} 
                    fill
                  ...
                  <div className="absolute top-3 sm:top-4 left-3 sm:left-4 px-2 sm:px-3 py-1 rounded-full bg-pink-500 text-white text-xs font-bold shadow-lg flex items-center gap-1">
                ...
                <div className="p-4 sm:p-6">
                  ...
                  <div className="flex items-end justify-between pt-4 border-t border-primary/5">
                    ...
                    <Link 
                      ...
                      className="px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl bg-pink-500 text-white text-xs sm:text-sm font-bold shadow-press hover:translate-y-0.5 transition-all"
                    >
                      Book Now
                    </Link>
                  </div>
